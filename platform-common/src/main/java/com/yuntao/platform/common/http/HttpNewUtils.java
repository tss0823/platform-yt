...
public class HttpNewUtils {
    ...
    final static RequestConfig requestConfig = RequestConfig.custom().setConnectTimeout(15000).setSocketTimeout(15000).build();
...
//    static Map<Res,byte []> contentMap = new ConcurrentHashMap();
    ...
    public static ResponseRes get(String url){
        ...
        try{
            ...
            BizException bizException = new BizException("http get failed!", e);
            if(e instanceof ConnectTimeoutException || e instanceof SocketTimeoutException){
                bizException.setCode(SystemConstant.ExceptionCode.REMOTE_TIME_OUT);
            }
            throw bizException;
        }finally {
            if(response != null){
